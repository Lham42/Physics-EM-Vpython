...
energy_wout_air = gdisplay(title = "Energy of the system vs Time",
                        x=500, y=200,   
                        xtitle = "time(s)",
                        ytitle = "energy(J)",
                        foreground = color.black,
...
y_pos_vs_x_pos_no_air_ball1 = gdots(gdisplay = trajectory, color = color.blue)        # a graphics curve for y position vs x position without air drag (less hangtime)
y_pos_vs_x_pos_no_air_ball2 = gdots(gdisplay = trajectory, color = color.red)        # a graphics curve for y position vs x position without air drag
y_pos_vs_x_pos_air_ball3 = gdots(gdisplay = trajectory, color = color.green)        # # a graphics curve for y position vs x position with airdrag      (less hangtime)
y_pos_vs_x_pos_air_ball4 = gdots(gdisplay = trajectory, color = color.yellow)        # # a graphics curve for y position vs x position with airdrag
     

#energy curves
KE_ball_1_v_t =  gdots(gdisplay= energy_wout_air, color = color.blue)               #kinetic
U_ball_1_v_t =   gdots(gdisplay= energy_wout_air, color = color.green)               #potential
ET_ball_1_v_t =   gdots(gdisplay= energy_wout_air, color = color.red)               #total
...
while(basketball1.pos.y > court.pos.y or basketball2.pos.y > court.pos.y or basketball3.pos.y > court.pos.y or basketball4.pos.y > court.pos.y ):
    ...
    #Energies
    KE = (mass*(mag(basketball1.mom/mass)**2))/2
    U = mass*9.8*basketball1.pos.y#dot(force_g,dr)
    ET = U + KE

    #plots
    if(basketball1.pos.y > court.pos.y):
        KE_ball_1_v_t.plot(pos = (t, KE))
        U_ball_1_v_t.plot(pos = (t, U))
        ET_ball_1_v_t.plot(pos = (t, ET))
